...
const RCON_CHAT_MESSAGE = 'CHAT_MESSAGE';

export default class Rcon {
	constructor(selectedServer, server) {
		this.host = selectedServer.ip;
		this.port = selectedServer.rconPort;
		this.password = selectedServer.password;
		...
		this.rconAutoReconnectInterval = 5000;
	...
	connect() {
		...
		return new Promise((resolve, reject) => {
			...
			this.client.on('error', (err) => {
				...
				this.server.emit(RCON_ERROR, err);
			});
